...
class TestHomework:
    ...
    def test_send_message(self, monkeypatch, random_sid):
        ...
        import homework_status

        assert hasattr(homework_status, 'send_message'), 'Функция `send_message()` не существует. Не удаляйте её.'
        assert hasattr(homework_status.send_message, '__call__'), 'Функция `send_message()` не существует. Не удаляйте её.'
        assert len(signature(homework_status.send_message).parameters) == 1, \
            'Функция `send_message()` должна быть с одним параметром.'

        result = homework_status.send_message('Test_message_check')
    ...
    def test_get_homework_statuses(self, monkeypatch, random_sid):
        ...
        assert hasattr(homework_status, 'get_homework_statuses'), \
            'Функция `get_homework_statuses()` не существует. Не удаляйте её.'
        assert hasattr(homework_status.get_homework_statuses, '__call__'), \
            'Функция `get_homework_statuses()` не существует. Не удаляйте её.'
        assert len(signature(homework_status.get_homework_statuses).parameters) == 1, \
            'Функция `get_homework_statuses()` должна быть с одним параметром.'

        result = homework_status.get_homework_statuses(234435234)
    ...
    def test_parse_homework_status(self, random_sid):
        ...
        assert hasattr(homework_status, 'parse_homework_status'), \
            'Функция `parse_homework_status()` не существует. Не удаляйте её.'
        assert hasattr(homework_status.parse_homework_status, '__call__'), \
            'Функция `parse_homework_status()` не существует. Не удаляйте её.'
        assert len(signature(homework_status.parse_homework_status).parameters) == 1, \
            'Функция `parse_homework_status()` должна быть с одним параметром.'

        result = homework_status.parse_homework_status(test_data)
